```ansi
[35mteam [34mlist [33m[[35m<[0mтима[35m>[33m]
```
Показывает все команды (далее - "тимы") с их отражаемыми именами и количество сущностей в них. Опциональный аргумент `тима` используется, чтобы указать одну тиму.

```ansi
[35mteam [34madd [33m<[0mтима[33m> [[35m<[0mотображаемое имя[35m>[33m]
```
Создаёт тиму с указанным именем в `тима`, опционально с отображаемым именем.

```ansi
[35mteam [34mremove [33m<[0mтима[33m>
```
Удаляет указанную тиму.

```ansi
[35mteam [34mempty [33m<[0mтима[33m>
```
Убирает всех участников с указанной тимы.

```ansi
[35mteam [34mjoin [33m<[0mтима[33m> [[35m<[36mселектор[35m>[33m]
```
Добавляет в тиму всех сущностей с `селектор`.

```ansi
[35mteam [34mleave [33m<[36mселектор[33m>
```
Убирвает указанных сущностей с их тим.

```ansi
team modify <тима> <настройка> <значение>
```
Модифицирует настройки указанной тимы.